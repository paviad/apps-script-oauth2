...
import { TOKEN_FORMAT, STORAGE_PREFIX_ } from './Consts';
...
/**
 * @file Contains the Service class.
 */
 ...
 * @private
 */
const EXPIRATION_BUFFER_SECONDS_ = 60;
 ...
 * @private
 */
const LOCK_EXPIRATION_MILLISECONDS_ = 30 * 1000;
...
export class Service {
   ...
   * @return {!Service} This service, for chaining.
   */
  ...
  generateCodeVerifier() {
    ...
    const verifier = utils.encodeUrlSafeBase64NoPadding_(rawBytes);
  ...
  getAuthorizationUrl(optAdditionalParameters) {
    ...
    if (optAdditionalParameters) {
      Object.keys(optAdditionalParameters).forEach(key => {
    ...
    if (this.codeVerifier_) {
      params['code_challenge'] = utils.encodeChallenge_(this.codeChallengeMethod_,
        this.codeVerifier_);
      params['code_challenge_method'] = this.codeChallengeMethod_;
    }
    params = utils.extend_(params, this.params_);
    return utils.buildUrl_(this.authorizationBaseUrl_, params);
  }
   ...
   */
  handleCallback(callbackRequest) {
  ...
  fetchToken_(payload, optUrl) {
    ...
    if (this.tokenHeaders_) {
      headers = utils.extend_(headers, this.tokenHeaders_);
    }
   ...
   * @private
   */
  getTokenFromResponse_(response) {
  ...
  ensureExpiresAtSet_(token) {
    ...
    var grantedTime = token.granted_time || utils.getTimeInSeconds_(new Date());
  ...
  isExpired_(token) {
    var expired = false;
    var now = utils.getTimeInSeconds_(new Date());

    // Check the authorization token's expiration.
    if (token.expiresAt) {
      if (token.expiresAt - now < EXPIRATION_BUFFER_SECONDS_) {
        expired = true;
      }
    }
    ...
    if (expiresIn) {
      ...
      if (expiresTime - now < EXPIRATION_BUFFER_SECONDS_) {
        expired = true;
      }
    }
    ...
    if (token.id_token) {
      var payload = utils.decodeJwt_(token.id_token);
      if (payload.exp &&
        payload.exp - now < EXPIRATION_BUFFER_SECONDS_) {
        expired = true;
      }
    }
  ...
  canRefresh_(token) {
    ...
    if (token.refreshTokenExpiresAt === undefined) {
      ...
      var now = utils.getTimeInSeconds_(new Date());
      return (
        token.refreshTokenExpiresAt - now > EXPIRATION_BUFFER_SECONDS_
      );
    }
  }
  ...
  createJwt_() {
    ...
    if (this.additionalClaims_) {
      ...
      Object.keys(additionalClaims).forEach(key => {
        claimSet[key] = additionalClaims[key];
      });
    }
    return utils.encodeJwt_(claimSet, this.privateKey_);
  }
  ...
  lockable_(func) {
    ...
    if (this.lock_ && !this.lock_.hasLock()) {
      this.lock_.waitLock(LOCK_EXPIRATION_MILLISECONDS_);
      releaseLock = true;
    }
  ...
  exchangeGrant_() {
    ...
    payload = utils.extend_(payload, this.params_);
    ...
    var lowerCaseHeaders = utils.toLowerCaseKeys_(this.tokenHeaders_);
